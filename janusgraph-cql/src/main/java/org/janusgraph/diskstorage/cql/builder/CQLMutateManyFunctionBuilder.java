    ...
    public CQLMutateManyFunction build(final CqlSession session, final Configuration configuration,
        ...
        if (atomicBatch) {
            mutateManyFunction = new CQLMutateManyLoggedFunction(times,
                assignTimestamp, openStores, session, sleepAfterWriteFunction, queriesBackPressure, executorService);
        } else {
            mutateManyFunction = new CQLMutateManyUnloggedFunction(batchSize,
                session, openStores, times, assignTimestamp, sleepAfterWriteFunction, queriesBackPressure, executorService);
        }
