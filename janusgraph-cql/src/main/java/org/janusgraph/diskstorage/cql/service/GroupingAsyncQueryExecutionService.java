...
public class GroupingAsyncQueryExecutionService implements AsyncQueryExecutionService {
    ...
    private final ResultFiller<Map<StaticBuffer, CompletableFuture<EntryList>>, SliceQuery, TokenRangeKeyGroup> SINGLE_QUERY_WITH_KEYS_GROUPING_FILLER;
    ...
    private final ResultFiller<Map<SliceQuery, Map<StaticBuffer, CompletableFuture<EntryList>>>, QueryGroups, TokenRangeKeyGroup> MULTI_QUERY_WITH_KEYS_GROUPING_FILLER;
    ...
    public GroupingAsyncQueryExecutionService(Configuration configuration,
        ...
        sliceGroupingLimit = Math.max(1, configuration.get(SLICE_GROUPING_LIMIT));
        keysGroupingLimit = Math.max(1, configuration.get(KEYS_GROUPING_LIMIT));
        keysGroupingMin = Math.max(2, configuration.get(KEYS_GROUPING_MIN));
        ...
        // @formatter:off
        final Select getSliceSelect = selectFrom(keyspaceName, edgeStoreTableName)
        ...
        if(sliceGroupingAllowed){
            // @formatter:off
            final Select getMultiColumnSelect = selectFrom(keyspaceName, edgeStoreTableName)
                .column(COLUMN_COLUMN_NAME)
                .column(VALUE_COLUMN_NAME)
                .where(
                    Relation.column(KEY_COLUMN_NAME).isEqualTo(bindMarker(KEY_BINDING)),
        ...
        if(keysGroupingAllowed){
            // @formatter:off
            final Select getMultiKeySliceSelect = selectFrom(keyspaceName, edgeStoreTableName)
            ...
            if(sliceGroupingAllowed){
                // @formatter:off
                final Select getMultiKeyMultiColumnSelect = selectFrom(keyspaceName, edgeStoreTableName)
                    .column(KEY_COLUMN_NAME)
                    .column(COLUMN_COLUMN_NAME)
                    .column(VALUE_COLUMN_NAME)
                    .where(
        ...
        groupedExecutionStrategy = GroupedExecutionStrategyBuilder.build(configuration, session, configuration.get(KEYS_GROUPING_CLASS));
    }
    ...
    private void fillMultiQueryWithKeysGrouping(final Map<SliceQuery, Map<StaticBuffer, CompletableFuture<EntryList>>> futureResult,
                                                ...
                                                TokenRangeKeyGroup tokenRangeKeyGroup,
                                                final StoreTransaction txh){
        ...
        for(Map.Entry<Integer, List<SliceQuery>> sliceQueriesGroup : queryGroups.getDirectEqualityGroupedQueriesByLimit().entrySet()){
            ...
            CompletableFuture<EntryList> multiKeyMultiColumnResult = cqlMultiKeyMultiColumnFunction.execute(new MultiKeysMultiColumnQuery(tokenRangeKeyGroup.getRoutingToken(), tokenRangeKeyGroup.getRawKeys(), queryStarts, limit), txh);
            Map<SliceQuery, Map<StaticBuffer, CompletableFuture<EntryList>>> partialResultToCompute = new HashMap<>(queryStarts.size());
            for(SliceQuery sliceQuery : sliceQueriesGroup.getValue()){
                Map<StaticBuffer, CompletableFuture<EntryList>> perKeyQueryPartialResult = new HashMap<>(tokenRangeKeyGroup.size());
                partialResultToCompute.put(sliceQuery, perKeyQueryPartialResult);
                Map<StaticBuffer, CompletableFuture<EntryList>> perKeyQueryFutureResult = futureResult.computeIfAbsent(sliceQuery, q -> new HashMap<>(tokenRangeKeyGroup.size()));
                for(StaticBuffer key : tokenRangeKeyGroup.getKeys()){
            ...
            multiKeyMultiColumnResult.whenComplete((entries, throwable) -> {
                if (throwable == null){
                    ...
                    for(Entry entry : entries){
                        ...
                        returnedResult.computeIfAbsent(query, q -> new HashMap<>(tokenRangeKeyGroup.size())).computeIfAbsent(key, k -> new EntryArrayList()).add(entry);
                    }
        ...
        for(SliceQuery separateQuery : queryGroups.getSeparateRangeQueries()){
            Map<StaticBuffer, CompletableFuture<EntryList>> perKeyQueryFutureResult = futureResult.computeIfAbsent(separateQuery, q -> new HashMap<>(tokenRangeKeyGroup.size()));
    ...
    private void fillSingleQueryWithKeysGrouping(final Map<StaticBuffer, CompletableFuture<EntryList>> futureQueryResult,
                                                 ...
                                                 final TokenRangeKeyGroup tokenRangeKeyGroup,
                                                 final StoreTransaction txh){

        CompletableFuture<EntryList> multiKeySingleSliceResult = cqlMultiKeySliceFunction.execute(new MultiKeysSingleSliceQuery(tokenRangeKeyGroup.getRoutingToken(), tokenRangeKeyGroup.getRawKeys(), query, query.getLimit()), txh);
        Map<StaticBuffer, CompletableFuture<EntryList>> perKeyQueryPartialResult = new HashMap<>(tokenRangeKeyGroup.size());
        for(StaticBuffer key : tokenRangeKeyGroup.getKeys()){
