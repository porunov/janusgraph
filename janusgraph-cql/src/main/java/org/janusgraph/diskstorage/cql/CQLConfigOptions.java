...
public interface CQLConfigOptions {
    ...
    ConfigOption<Boolean> KEYS_GROUPING_ALLOWED = new ConfigOption<>(
        ...
        "If `true` this allows multiple partition keys to be grouped together into a single CQL query via `IN` operator based on the keys grouping strategy provided " +
            "(usually grouping is done by same token-ranges, but may also involve shard ids for custom implementations).\n" +
            ...
            "Notice, the default grouping strategy is not taking into account shards. Thus, this might be inefficient with ScyllaDB storage backend. " +
    ...
    ConfigOption<String> KEYS_GROUPING_CLASS = new ConfigOption<>(
        ...
        "Full class path of the keys grouping execution strategy. The class should implement " +
            ...
            "with two arguments `org.janusgraph.diskstorage.configuration.Configuration` and `com.datastax.oss.driver.api.core.CqlSession`.\n" +
            "Shortcuts available:\n" +
            "- `"+GroupedExecutionStrategyBuilder.DEFAULT_CLASS+"` - groups partition keys which belong to the same token range. Notice, this strategy is not taking into account shards. Thus, this might be inefficient with ScyllaDB storage backend.\n" +
            "\nThis option takes effect only when `"+KEYS_GROUPING_ALLOWED.toStringWithoutRoot()+"` is `true`.",
        ConfigOption.Type.MASKABLE,
        GroupedExecutionStrategyBuilder.DEFAULT_CLASS);

    ConfigOption<Integer> KEYS_GROUPING_LIMIT = new ConfigOption<>(
        CQL_GROUPING_NS,
        "keys-limit",
        "Maximum amount of the keys which belong to the same token range to be grouped together into a single CQL query. " +
            "If more keys for the same token range are queried, they are going to be grouped into separate CQL queries.\n"
            + MAX_IN_CONFIG_MSG +
            "\nThis option takes effect only when `"+KEYS_GROUPING_ALLOWED.toStringWithoutRoot()+"` is `true`.",
        ConfigOption.Type.MASKABLE,
        20);
